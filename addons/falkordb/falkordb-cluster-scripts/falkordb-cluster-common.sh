...
shutdown_redis_server() {
  ...
  if ! is_empty "$REDIS_DEFAULT_PASSWORD"; then
    redis-cli $(redis_cli_tls_flag) -h 127.0.0.1 -p "$service_port" -a "$REDIS_DEFAULT_PASSWORD" shutdown
  else
    redis-cli $(redis_cli_tls_flag) -h 127.0.0.1 -p "$service_port" shutdown
  fi
...
check_redis_server_ready() {
  ...
  check_ready_cmd="redis-cli $(redis_cli_tls_flag) -h $host -p $port ping"
  if ! is_empty "$REDIS_DEFAULT_PASSWORD"; then
    check_ready_cmd="redis-cli $(redis_cli_tls_flag) -h $host -p $port -a $REDIS_DEFAULT_PASSWORD ping"
  fi
  set_xtrace_when_ut_mode_false
...
send_cluster_meet() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    meet_command="redis-cli $(redis_cli_tls_flag) -h $primary_endpoint -p $primary_port cluster meet $announce_ip $announce_port $announce_bus_port"
    logging_mask_meet_command="$meet_command"
  else
    meet_command="redis-cli $(redis_cli_tls_flag) -h $primary_endpoint -p $primary_port -a $REDIS_DEFAULT_PASSWORD cluster meet $announce_ip $announce_port $announce_bus_port"
...
get_cluster_info() {
  ...
  unset_xtrace_when_ut_mode_false
  local command="redis-cli $(redis_cli_tls_flag) -h $cluster_node -p $cluster_node_port cluster info"
  if ! is_empty "$REDIS_DEFAULT_PASSWORD"; then
    command="redis-cli $(redis_cli_tls_flag) -h $cluster_node -p $cluster_node_port -a $REDIS_DEFAULT_PASSWORD cluster info"
  fi
  set_xtrace_when_ut_mode_false
...
get_cluster_nodes_info() {
  ...
  local cluster_node_port="$2"
  unset_xtrace_when_ut_mode_false
  local command="redis-cli $(redis_cli_tls_flag) -h $cluster_node -p $cluster_node_port cluster nodes"
  if ! is_empty "$REDIS_DEFAULT_PASSWORD"; then
    command="redis-cli $(redis_cli_tls_flag) -h $cluster_node -p $cluster_node_port -a $REDIS_DEFAULT_PASSWORD cluster nodes"
  fi
  set_xtrace_when_ut_mode_false
...
check_slots_covered() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    check=$(redis-cli $(redis_cli_tls_flag) --cluster check "$node_endpoint_wth_port" -p "$cluster_service_port")
  else
    check=$(redis-cli $(redis_cli_tls_flag) --cluster check "$node_endpoint_wth_port" -p "$cluster_service_port" -a "$REDIS_DEFAULT_PASSWORD" )
  fi
  set_xtrace_when_ut_mode_false
...
build_redis_cluster_create_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    initialize_command="redis-cli $(redis_cli_tls_flag) --cluster create $primary_nodes --cluster-yes"
    logging_mask_initialize_command="$initialize_command"
  else
    initialize_command="redis-cli $(redis_cli_tls_flag) --cluster create $primary_nodes -a $REDIS_DEFAULT_PASSWORD --cluster-yes"
...
build_secondary_replicated_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    replicated_command="redis-cli $(redis_cli_tls_flag) --cluster add-node $secondary_endpoint_with_port $mapping_primary_endpoint_with_port --cluster-slave --cluster-master-id $mapping_primary_cluster_id"
    logging_mask_replicated_command="$replicated_command"
  else
    replicated_command="redis-cli $(redis_cli_tls_flag) --cluster add-node $secondary_endpoint_with_port $mapping_primary_endpoint_with_port --cluster-slave --cluster-master-id $mapping_primary_cluster_id -a $REDIS_DEFAULT_PASSWORD"
...
build_scale_out_shard_primary_join_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    add_node_command="redis-cli $(redis_cli_tls_flag) --cluster add-node $scale_out_shard_default_primary_endpoint_with_port $exist_available_node"
    logging_mask_add_node_command="$add_node_command"
  else
    add_node_command="redis-cli $(redis_cli_tls_flag) --cluster add-node $scale_out_shard_default_primary_endpoint_with_port $exist_available_node -a $REDIS_DEFAULT_PASSWORD"
...
build_reshard_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    reshard_command="redis-cli $(redis_cli_tls_flag) --cluster reshard $primary_node_with_port --cluster-from all --cluster-to $mapping_primary_cluster_id --cluster-slots $slots_per_shard --cluster-yes"
    logging_mask_reshard_command="$reshard_command"
  else
    reshard_command="redis-cli $(redis_cli_tls_flag) --cluster reshard $primary_node_with_port --cluster-from all --cluster-to $mapping_primary_cluster_id --cluster-slots $slots_per_shard -a $REDIS_DEFAULT_PASSWORD --cluster-yes"
...
build_rebalance_to_zero_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    rebalance_command="redis-cli $(redis_cli_tls_flag) --cluster rebalance $node_with_port --cluster-weight $node_cluster_id=0 --cluster-yes "
    logging_mask_rebalance_command="$rebalance_command"
  else
    rebalance_command="redis-cli $(redis_cli_tls_flag) --cluster rebalance $node_with_port --cluster-weight $node_cluster_id=0 --cluster-yes -a $REDIS_DEFAULT_PASSWORD"
...
build_del_node_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    del_node_command="redis-cli $(redis_cli_tls_flag) --cluster del-node $available_node $node_to_del_cluster_id -p $SERVICE_PORT"
    if [[ "$do_forget_node" == "true" ]]; then
      del_node_command="redis-cli $(redis_cli_tls_flag) -p $SERVICE_PORT --cluster call $available_node cluster forget $node_to_del_cluster_id"
    fi
    logging_mask_del_node_command="$del_node_command"
  else
    del_node_command="redis-cli $(redis_cli_tls_flag) --cluster del-node $available_node $node_to_del_cluster_id -p $SERVICE_PORT -a $REDIS_DEFAULT_PASSWORD"
    if [[ "$do_forget_node" == "true" ]]; then
      del_node_command="redis-cli $(redis_cli_tls_flag) -p $SERVICE_PORT --cluster call $available_node cluster forget $node_to_del_cluster_id -a $REDIS_DEFAULT_PASSWORD"
    fi
...
build_acl_save_command() {
  ...
  unset_xtrace_when_ut_mode_false
  if ! is_empty "$REDIS_DEFAULT_PASSWORD"; then
    acl_save_command="redis-cli $(redis_cli_tls_flag) -h localhost -p $service_port -a $REDIS_DEFAULT_PASSWORD acl save"
    logging_mask_acl_save_command="${acl_save_command/$REDIS_DEFAULT_PASSWORD/********}"
  else
    acl_save_command="redis-cli $(redis_cli_tls_flag) -h localhost -p $service_port acl save"
...
check_redis_role() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    role_info=$(redis-cli $(redis_cli_tls_flag) -h $host -p $port info replication)
  else
    role_info=$(redis-cli $(redis_cli_tls_flag) -h $host -p $port -a "$REDIS_DEFAULT_PASSWORD" info replication)
  fi
  set_xtrace_when_ut_mode_false
