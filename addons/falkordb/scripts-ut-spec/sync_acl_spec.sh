...
Describe "FalkorDB Sync ACL Script Tests"
  ...
  Describe "fetch_acl_list_from_peers()"
    Context "when a peer returns ACL LIST successfully"
      ...
      It "returns the ACL list"
        When call fetch_acl_list_from_peers "falkordb-0.svc,falkordb-1.svc" "falkordb-1.svc" "redis-cli -p 6379"
    ...
    Context "when self is the only pod"
      ...
      It "fails because no peer is available"
        When call fetch_acl_list_from_peers "falkordb-0.svc" "falkordb-0.svc" "redis-cli -p 6379"
    ...
    Context "when all peers fail"
      ...
      It "fails with error message"
        When call fetch_acl_list_from_peers "falkordb-0.svc,falkordb-1.svc" "falkordb-0.svc" "redis-cli -p 6379"
        The status should be failure
        The stderr should include "Failed to get ACL LIST from other pods"
      End
    End
    ...
    Context "when first peer fails but second succeeds"
      redis-cli() {
        ...
        if echo "$*" | grep -q "ACL LIST"; then
          if [ "$host_arg" = "falkordb-1.svc" ]; then
            return 1
          fi
      ...
      It "skips self, retries peers, and returns from successful one"
        When call fetch_acl_list_from_peers "falkordb-0.svc,falkordb-1.svc,falkordb-2.svc" "falkordb-0.svc" "redis-cli -p 6379"
        The status should be success
  ...
  Describe "apply_acl_rules()"
    Context "with valid non-default user rules"
      ...
      It "applies ACL SETUSER for non-default users and runs ACL save"
        ...
        When call apply_acl_rules "$acl_list" "falkordb-new.svc" "redis-cli -p 6379"
        The status should be success
