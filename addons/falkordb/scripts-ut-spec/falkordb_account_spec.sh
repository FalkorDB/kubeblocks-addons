...
if ! validate_shell_type_and_version "bash" 4 &>/dev/null; then
  echo "falkordb_account_spec.sh skip all cases because dependency bash version 4 or higher is not installed."
  exit 0
fi

Describe "FalkorDB Account Script Tests"
  Include ../scripts/falkordb-account.sh
  ...
  Describe "env_pre_check()"
    ...
    Context "when shard mode and CURRENT_SHARD_COMPONENT_NAME is empty"
      setup() {
        ...
        export REDIS_DEFAULT_USER="default"
        export SHARD_MODE="TRUE"
        export CURRENT_POD_NAME="falkordb-shard-0"
    ...
    Context "when shard mode and CLUSTER_NAMESPACE is empty"
      setup() {
        ...
        export SHARD_MODE="TRUE"
        export CURRENT_POD_NAME="falkordb-shard-0"
        export CURRENT_SHARD_COMPONENT_NAME="falkordb-shard"
    ...
    Context "when non-shard mode with all required vars set"
      setup() {
        ...
        export REDIS_DEFAULT_USER="default"
        export SHARD_MODE="FALSE"
        export REDIS_POD_FQDN_LIST="falkordb-0.falkordb-headless.ns.svc.cluster.local"
      }
      Before "setup"
  ...
  Describe "do_acl_command()"
    Context "with FQDN host and successful ACL operations"
      ...
      It "executes ACL command and save for each host"
        ...
        service_port=6379
        When run do_acl_command "falkordb-0.falkordb-headless.ns.svc.cluster.local" "default" "password123"
        The status should be success
    ...
    Context "with multiple FQDN hosts"
      ...
      It "iterates all hosts"
        ...
        export REDIS_CLI_TLS_CMD=""
        service_port=6379
        When run do_acl_command "falkordb-0.falkordb-headless.ns.svc,falkordb-1.falkordb-headless.ns.svc" "default" "password123"
        The status should be success
        The stdout should include "DO ACL COMMAND FOR ALL HOSTS SUCCESS"
      End
    End
    ...
    Context "when ACL command returns ERR"
      ...
      It "exits with failure"
        ...
        export REDIS_CLI_TLS_CMD=""
        service_port=6379
        When run do_acl_command "falkordb-0.falkordb-headless.ns.svc" "default" "password123"
        The status should be failure
        The stdout should include "DO ACL COMMAND FOR HOST: falkordb-0.falkordb-headless.ns.svc FAILED"
    ...
    Context "when ACL SAVE fails"
      ...
      It "exits with failure"
        ...
        When run do_acl_command "falkordb-0.falkordb-headless.ns.svc" "default" "password123"
        The status should be failure
        The stdout should include "DO ACL SAVE FOR HOST: falkordb-0.falkordb-headless.ns.svc FAILED"
      End
    End
    ...
    Context "when password is empty"
      ...
      It "builds command without -a flag"
        ...
        export REDIS_CLI_TLS_CMD=""
        service_port=6379
        When run do_acl_command "falkordb-0.falkordb-headless.ns.svc" "default" ""
        The status should be success
        The stdout should include "DO ACL COMMAND FOR ALL HOSTS SUCCESS"
      End
    End
  ...
  Describe "main()"
    Context "in non-shard mode with all env vars set"
      ...
      It "runs ACL command on REDIS_POD_FQDN_LIST hosts"
        ...
        export REDIS_DEFAULT_PASSWORD="password123"
        export SHARD_MODE="FALSE"
        export REDIS_POD_FQDN_LIST="falkordb-0.falkordb-headless.ns.svc"
        export REPLICAS=1
        export REDIS_CLI_TLS_CMD=""
        service_port=6379
        When run main
