...
#FalkorDBParameter: {
    ...
    // FalkorDB default starting with FalkorDB 3.2.1.
    ...
    // FalkorDB will save the DB if the given number of seconds elapsed and it
    // surpassed the given number of write operations against the DB.
    // Snapshotting can be completely disabled with a single empty string argument
    // as in following example:
    // save ""
    // Unless specified otherwise, by default FalkorDB will save the DB:
    //   * After 3600 seconds (an hour) if at least 1 change was performed
    //   * After 300 seconds (5 minutes) if at least 100 changes were performed
    //   * After 60 seconds if at least 10000 changes were performed
    save: string | *"3600 1 300 100 60 10000"

    // By default FalkorDB will stop accepting writes if RDB snapshots are enabled
    // (at least one save point) and the latest background save failed.
    // This will make the user aware (in a hard way) that data is not persisting
    // on disk properly, otherwise chances are that no one will notice and some
    // disaster will happen.
    // If the background saving process will start working again FalkorDB will
    // automatically allow writes again.
    // However if you have setup your proper monitoring of the FalkorDB server
    // and persistence, you may want to disable this feature so that FalkorDB will
    ...
    // Starting from FalkorDB 5, by default a replica will ignore its maxmemory setting
    ...
    // FalkorDB reclaims expired keys in two ways: upon access when those keys are
    ...
    // FalkorDB calls an internal function to perform many background tasks, like
    // closing connections of clients in timeout, purging expired keys that are
    // never requested, and so forth.
    // Not all tasks are performed with the same frequency, but FalkorDB checks for
    // tasks to perform according to the specified "hz" value.
    // By default "hz" is set to 10. Raising the value will use more CPU when
    // FalkorDB is idle, but at the same time will make FalkorDB more responsive when
    ...
    // FalkorDB Enterprise module — graph offloading

    "falkordbe.offload-path": string | *""

    "falkordbe.idle-threshold-ms": int & >=0 | *0

    "falkordbe.evict-interval-ms": int & >=1000 | *10000

    "falkordbe.memory-pressure": number & >=0 & <=1 | *0

    "falkordbe.load-evict-budget": int & >=-1 | *0

    // FalkorDB Enterprise module — LDAP authentication

    "falkordbe.ldap_servers": string | *""

    "falkordbe.ldap_auth_mode": string & "bind" | "search+bind" | *"bind"

    "falkordbe.ldap_bind_dn_prefix": string | *""

    "falkordbe.ldap_bind_dn_suffix": string | *""

    "falkordbe.ldap_search_base": string | *""

    "falkordbe.ldap_search_filter": string | *""

    "falkordbe.ldap_search_scope": string & "base" | "one" | "sub" | *"sub"

    "falkordbe.ldap_search_attribute": string | *""

    "falkordbe.ldap_search_dn_attribute": string | *""

    "falkordbe.ldap_search_bind_dn": string | *""

    "falkordbe.ldap_search_bind_passwd": string | *""

    "falkordbe.ldap_connection_pool_size": int & >=1 & <=8192 | *2

    "falkordbe.ldap_timeout_connection": int & >=0 | *2

    "falkordbe.ldap_timeout_ldap_operation": int & >=0 | *2

    "falkordbe.ldap_failure_detector_interval": int & >=0 | *1

    "falkordbe.ldap_use_starttls": string & "yes" | "no" | *"no"

    "falkordbe.ldap_tls_ca_cert_path": string | *""

    "falkordbe.ldap_tls_cert_path": string | *""

    "falkordbe.ldap_tls_key_path": string | *""

    "falkordbe.ldap_tls_skip_verify": string & "yes" | "no" | *"no"

    "falkordbe.ldap_exempted_users_regex": string | *""

    "falkordbe.ldap_return_auth_errors": string & "yes" | "no" | *"no"

    "falkordbe.ldap_acl_fallback_enabled": string & "yes" | "no" | *"no"

    "falkordbe.ldap_default_acl_rules": string | *"on resetpass"

    "falkordbe.ldap_groups_search_base": string | *""

    "falkordbe.ldap_groups_filter": string | *"objectClass=groupOfNames"

    "falkordbe.ldap_groups_name_attribute": string | *"cn"

    "falkordbe.ldap_groups_member_attribute": string | *"member"

    "falkordbe.ldap_groups_rules_attribute": string | *"valkeyACL"
