...
get_master_addr_by_name(){
  ...
  if is_empty "$ip_addr" || echo "$ip_addr" | grep -E '^([a-zA-Z0-9-]+\.[a-zA-Z0-9-]+\.default\.svc(\.[a-zA-Z0-9.-]+)?|([0-9]{1,3}\.){3}[0-9]{1,3})$' > /dev/null; then
    echo "$output" 
    return 0
  else
...
register_to_sentinel() {
  ...
  # Check if Sentinel is already monitoring the FalkorDB primary
  ...
  if is_empty "$master_addr"; then
    ...
    # Register the FalkorDB primary with Sentinel
