  ...
  if [[ -n "$output" ]]; then
    ...
    while read -r line; do
      ...
      if [[ -n "$master_name" ]]; then
        ...
        redis-cli $(redis_cli_tls_flag) -h "$sentinel_leave_member_fqdn" -p "$redis_default_service_port" -a "$SENTINEL_PASSWORD" SENTINEL REMOVE "$master_name"
...
redis_sentinel_reset_all() {
  for sentinel_pod in "${sentinel_pod_list[@]}"; do
    ...
    if [ "$sentinel_name" != "$sentinel_leave_member_name" ]; then
      ...
      while [ $retry_count -lt $max_retries ]; do
        if [ -n "$SENTINEL_PASSWORD" ]; then
          if redis-cli $(redis_cli_tls_flag) -h "$host" -p "$redis_default_service_port" -a "$SENTINEL_PASSWORD" SENTINEL RESET "*"; then
            echo "sentinel is resetting at $host on port $redis_default_service_port."
            success=true
            break
          fi
        else
          if redis-cli $(redis_cli_tls_flag) -h "$host" -p "$redis_default_service_port" SENTINEL RESET "*" ; then
            echo "sentinel is resetting at $host on port $redis_default_service_port."
            success=true
            break
          fi
