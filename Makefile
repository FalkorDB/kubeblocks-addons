...
E2E_TEST ?=
E2E_PARALLEL ?= 4
# Label query passed to chainsaw. Scenarios that cannot run unattended are
# labelled `e2e.falkordb/ci: unsupported`, so CI sets
# E2E_SELECTOR='e2e.falkordb/ci!=unsupported' to leave them out. A `!=` selector
# also matches tests that carry no such label, so nothing else is affected.
# Empty by default: a local run executes everything.
E2E_SELECTOR ?=
...
e2e: install-chainsaw ##    Run the chainsaw e2e suite against the current kubecontext.
	@$(CHAINSAW) test \
		...
		$(if $(E2E_SELECTOR),--selector '$(E2E_SELECTOR)') \
